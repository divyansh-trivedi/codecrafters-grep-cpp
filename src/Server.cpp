...
int match_recursive(const string& pattern, const string& text) {
    ...
    if (pattern[0] == '(') {
        ...
        if (end_paren_pos != string::npos) {
            ...
            if (quantifier == '+') {
                // Must match group at least once
                int group_len = match_recursive(group_content, text);
                if (group_len == -1) return -1;

                string remaining_text = text.substr(group_len);
                string rest_of_pattern = after_group.substr(1);
                string quantified_group = pattern.substr(0, end_paren_pos + 2); // e.g. "(...)+"

                // Path A (greedy): Try to match the group+ again on the remaining text
                int more_len = match_recursive(quantified_group, remaining_text);
                if (more_len != -1) return group_len + more_len;
    ...
    if (pattern.length() > 1 && (pattern[1] == '?' || pattern[1] == '+')) {
        ...
        if (!text.empty() && (pattern[0] == '.' || pattern[0] == text[0])) {
            if (pattern[1] == '+') {
                // Path A (greedy): Match one and then try to match `+` again
                int len = match_recursive(pattern, text.substr(1));
                if (len != -1) return 1 + len;
            }
            // Path B (backtrack): Match one and then match the rest of the pattern
            int len = match_recursive(rest, text.substr(1));
            if (len != -1) return 1 + len;
        }
        return -1;
    }
