...
int match_recursive(const string& pattern, const string& text) {
    ...
    if (pattern[0] == '(') {
        ...
        if (end_paren_pos != string::npos) {
            ...
            if (quantifier == '+') {
                ...
                string pattern_after_plus = after_group.substr(1);

                // =================================================================
                // START OF CHANGE: Swapped the order to make the '+' greedy
                // =================================================================
                // Path B (Greedy): Try to match the quantified group again first.
                int more_len = match_recursive(pattern.substr(0, end_paren_pos + 2), text_after_one);
                if (more_len != -1) return len1 + more_len;
                
                // Path A (Backtrack): If the greedy path failed, now try matching the rest.
    ...
    if (pattern.length() > 1 && (pattern[1] == '?' || pattern[1] == '+')) {
        ...
        if (pattern[1] == '?') return match_recursive(pattern.substr(2), text);
        return -1;
    }
